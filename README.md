...
**이하 내용 부터는 게임에 필요한 개발 요소들을 주차별로 나눈 자료입니다.**
  ...
  1주차
    ...
    -패에 넣을 이미지 리소스 구하기(10가지)
    -패 선택 시 테두리 진해지는 효과
    -서로 지울 수 있는 패인지 판단(알고리즘)
    
  2주차
  
    -지울 수 있는 패 (가능패) 수를 나타내도록 시스템 추가
    -콤보 기능 추가 (5콤보 시 강제로 패를 하나 지울 수 있도록)
    -제한 시간 추가 및 시각화
  ...
  3주차
    ...
    -Highscore 추가
    -특이한 속성의 패 추가
      -열쇠모양의 패로 지울 경우 막혀있던 벽을 없앰
      -숫자모양의 패로 이전 숫자(2일 경우 1을) 패를 지워야만 다음 숫자 패를 지울 수 있음
      -주변 패가 깨질 경우 활성화되는 얼음 
